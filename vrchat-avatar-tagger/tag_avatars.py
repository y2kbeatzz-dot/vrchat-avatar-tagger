...
USAGE:
  ...
  python3 tag_avatars.py --tags sex --remove                  # REMOVE this tag instead of adding it
...
IMPORTANT NOTES BEFORE YOU RUN THIS:
  ...
  - By default this script only ADDS the tags you specify, and never
    touches any other tags already on an avatar. Pass --remove to instead
    remove the specified tags (still leaving every other tag untouched).
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--remove",
        action="store_true",
        help="Remove the specified tag(s) instead of adding them. All other tags are left untouched.",
    )
    ...
    return [CONTENT_TAGS[n] for n in names], args.dry_run, args.select, args.limit, args.remove
...
def main():
    tags_to_apply, dry_run, select_mode, limit, remove_mode = parse_args()
    ...
    for i, avatar in enumerate(avatars_to_process, 1):
        ...
        if new_tags == existing_tags:
            ...
            print(f"[{i}/{total}] {avatar.name} -- {reason}, skipping")
            skipped += 1
            continue

        print(f"[{i}/{total}] {avatar.name} -- {action_word} {tags_to_apply}")
